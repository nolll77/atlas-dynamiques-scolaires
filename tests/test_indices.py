...
def test_gini_increases_with_inequality():
    """Gini augmente avec l'inégalité."""
    equal = np.ones(100) * 150.0
    unequal = np.concatenate([np.ones(50) * 100, np.ones(50) * 200])
    assert gini_index(unequal) > gini_index(equal)
...
def test_entre_soi_symmetry():
    """Score d'entre-soi = 0 pour valeurs moyennes exactes."""
    score = entre_soi_score(
        ips=150, sigma=25, ips_mean=150, ips_std=10, sigma_mean=25, sigma_std=5
    )
