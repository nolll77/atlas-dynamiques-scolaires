...
def theil_index(df: pd.DataFrame, group_col: str, value_col: str) -> dict:
    ...
    total = float(np.sum((values / mu) * np.log(values / mu)) / n)
    ...
    for _, group_values in groups:
        gv = group_values.values.astype(float)
        nk = len(gv)
        mu_k = np.mean(gv)
        weight = nk / n
        within += weight * float(np.sum((gv / mu_k) * np.log(gv / mu_k)) / nk)
    ...
    return {
        ...
        "within": within,
        "between": between,
        "ratio_within": within / total if total > 0 else 0.0,
        "ratio_between": between / total if total > 0 else 0.0,
    }
...
def duncan_dissimilarity(
    ...
    return float(
        0.5 * np.sum(np.abs(groups[col_a] / total_a - groups[col_b] / total_b))
    )
